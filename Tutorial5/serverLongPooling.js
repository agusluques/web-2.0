...
fs.readFile('./index.html', function (err, html) {
 ...
 const server = http.createServer(function(request, response) { 
 	if (request.method === 'GET' && request.url.startsWith('/messages')) {
        ...
        let token = getUrlParameter(request.url, 'token');
        areNewMessages = (token) => {
            for (var i = 0; i < messages.length; i++) {
            if (messages[i].sender !== token && !messages[i].sent.find(e => e === token)) return true;
        }; return false;}

        var checkExist = setInterval(function() {
            if (areNewMessages(token)) {
                var aux = []
                for (var i = 0; i < messages.length; i++) {
